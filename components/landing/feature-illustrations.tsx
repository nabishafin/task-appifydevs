...
export function ModelSwitcherIllustration() {
  return (
    <div aria-hidden="true" className="rounded-xl bg-card/90 p-2 shadow-sm">
        ...
        {SWITCHER_MODELS.map((model, index) => {
          ...
          return (
            ...
            >
              ...
              {model.tier === "pro" && (
                <span className="rounded-md bg-muted px-1.5 text-[10px] text-muted-foreground">Pro</span>
              )}
...
export function MixedThreadIllustration({ className }: { className?: string }) {
  return (
    <ol
      aria-hidden="true"
      className={cn("flex-col justify-center gap-2 rounded-xl bg-card/90 p-3 shadow-sm", className)}
    >
...
export function CompareIllustration() {
  ...
  return (
    <div aria-hidden="true" className="grid grid-cols-2 gap-2">
      {columns.map((column) => (
        <div key={column.name} className="rounded-xl bg-card/90 p-2.5 shadow-sm">
  ...
  return (
    <ul aria-hidden="true" className="flex flex-wrap gap-1.5">
      {QUICK_ACTIONS.map(({ id, label, icon: Icon }) => (
        <li
          key={id}
          className="inline-flex h-7 items-center gap-1.5 rounded-md bg-card/90 px-2.5 text-[11px] font-medium text-muted-foreground shadow-xs"
        >
